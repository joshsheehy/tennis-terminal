-- Lightweight storage for public ATP acceptance/entry lists.
-- We intentionally store the ordered list as JSONB instead of one row per player.
-- Challenger lists are small, and snapshots are written only when the source file
-- changes, so this keeps both database size and write volume tiny while preserving
-- enough history to reconstruct list movement later.
